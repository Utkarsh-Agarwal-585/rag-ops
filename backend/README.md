...
| Structured logging | Python stdlib (JSON formatter) |
...
backend/
└── app/
    ├── main.py                  # FastAPI app factory, CORS, static mounts, lifespan
    ├── config.py                # All tunable constants (single source of truth)
    ├── logging_config.py        # JSON structured logging + correlation ID ContextVar
    ├── middleware/
...
Every request gets an `x-correlation-id` header in the response (generated by `middleware/correlation.py`). All log output is JSON-structured with the correlation ID embedded:

```json
{"timestamp": "2025-05-03T12:34:56.789Z", "level": "INFO", "logger": "app.services.ingestion.ingestor", "message": "Ingestion complete", "correlation_id": "req_a3f2b1c9_1746268496_0042", "module": "ingestor", "line": 87}
```

To trace a failed request: copy `x-correlation-id` from the browser's Network tab → grep your logs for it.

---

## Persistence

After every successful upload, the full retrieval state is saved to `backend/storage/index/`:

| File | Contents |
|---|---|
| `chunks.pkl` | All chunk objects + ID index + source index |
| `faiss.index` | FAISS binary index |
| `faiss_ids.pkl` | Chunk ID → FAISS position mapping |
| `bm25.pkl` | BM25 token corpus + chunk IDs |

On server startup, these are loaded automatically. No re-upload needed after restarts.
